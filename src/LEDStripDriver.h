...
public:
  ...
  void init(uint16_t numPixels, uint8_t brightness,
   ...
   *
   * @param colors Comma-separated hex color codes (e.g.,
  ...
  /**
   * @brief Returns the last set color pattern. If the LED strip is currently
   * off, returns the last colors before it was turned off.
   *
   * @return Comma-separated hex color string.
   */
...
private:
  ...
  String getLastColorsFromStorage() const;
};
